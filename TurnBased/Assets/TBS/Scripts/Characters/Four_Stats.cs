...
using UnityEngine;
public class Four_Stats : MonoBehaviour {
    public static Four_Stats instance;
    [HideInInspector]
    ...
    public void GetStats() {
        level = Start_Battle.instance.four.level;
        thisName = this.gameObject.ToString();
        stats = GetCharacterStats(Start_Battle.instance.four, Start_Battle.instance.four.level);
        CreateTurn();
        health = stats[0];
        maxHealth = health;
        magic = stats[1];
