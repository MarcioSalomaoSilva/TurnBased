...
using UnityEngine;
public class Two_Stats : MonoBehaviour {
    public static Two_Stats instance;
    [HideInInspector]
    ...
    public void GetStats() {
        level = Start_Battle.instance.two.level;
        thisName = this.gameObject.ToString();
        stats = GetCharacterStats(Start_Battle.instance.two, Start_Battle.instance.two.level);
        CreateStats();
        health = stats[0];
        maxHealth = health;
        magic = stats[1];
